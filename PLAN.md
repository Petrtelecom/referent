...
- ✅ Переменная окружения `TRANSLATION_API_KEY` уже используется
...
- Проверка наличия `TRANSLATION_API_KEY`
...
- `TRANSLATION_API_KEY` - уже используется, дополнительных не требуется
